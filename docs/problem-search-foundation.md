...
The tutor request accepts only `POST` JSON and ignores provider configuration from the browser. A current-problem request may include a session transcript for continuity, but the server still rebuilds the trusted problem context from the HackerBlocks source endpoint.
