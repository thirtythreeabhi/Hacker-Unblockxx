...
function isArtifactType(value: unknown): value is ArtifactType {
  return (
    typeof value === "string" && ARTIFACT_TYPES.includes(value as ArtifactType)
  );
}
...
async function loadQuestion(contestId: string, contentId: string) {
  ...
  if (!upstream.ok)
    throw new Error(`Question could not be fetched (HTTP ${upstream.status})`);
...
async function cachedArtifact(
  ...
  let query = client
    .from("ai_problem_artifacts")
    .select(
      "id,problem_id,content_id,artifact_type,language,source_hash,payload,model,created_at,updated_at",
    )
...
export async function POST(request: Request) {
  ...
  if (
    !isNumericId(contestId) ||
    !isNumericId(contentId) ||
    !isArtifactType(artifactType)
  )
    return NextResponse.json(
      { error: "contestId, contentId, and a valid artifactType are required." },
      { status: 400 },
    );
  ...
  const language =
    artifactType === "boilerplate"
      ? typeof body?.language === "string" &&
        ...
        : "cpp"
      : null;
  if (artifactType === "boilerplate" && body?.language && !language)
    return NextResponse.json(
      { error: "Unsupported boilerplate language." },
      { status: 400 },
    );
  ...
  try {
    ...
    const generation = async () => {
      const generated = await generateWithGemini({
        task: artifactType as GeminiTask,
        input: sourceInput(question),
        language: language ?? undefined,
      });
      const payload = validateArtifact(
        artifactType as GeminiTask,
        generated.payload,
        language ?? undefined,
      );
      ...
      const { data, error } = await admin
        .from("ai_problem_artifacts")
        .upsert(record, {
          onConflict: "problem_id,artifact_type,language_key,source_hash",
        })
        .select("payload,model,source_hash")
        .single();
      if (error) throw new Error(error.message);
      return {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "AI artifact could not be generated.";
    const status = /log in/i.test(message)
      ? 401
      : /Question could not be fetched/.test(message)
        ? 502
        : 500;
