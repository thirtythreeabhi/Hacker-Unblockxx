...
const MODES = new Set<RecommendationMode>([
  "continue",
  "harder",
  "bookmarked",
  "random",
...
function inputFrom(value: Record<string, unknown>): Input {
  const rawMode =
    typeof value.mode === "string"
      ? (value.mode.toLowerCase() as RecommendationMode)
      : "random";
  const rawLimit = Number.parseInt(String(value.limit ?? "5"), 10);
  return {
    mode: MODES.has(rawMode) ? rawMode : "random",
    currentProblemId: /^\d+$/.test(String(value.currentProblemId ?? ""))
      ? String(value.currentProblemId)
      : null,
    topic:
...
async function readNearDuplicates() {
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter(
          (pair): pair is NearDuplicatePair =>
            /^\d+$/.test(String(pair?.problemIdA)) &&
            /^\d+$/.test(String(pair?.problemIdB)),
        )
      : [];
  } catch (error) {
    if (
...
async function readProblems(
  ...
  const columns = includeEmbeddings
    ? "problem_id,content_id,contest_id,name,difficulty,topics,primary_topics,domain,embedding"
    : "problem_id,content_id,contest_id,name,difficulty,topics,primary_topics,domain";
  ...
  for (let offset = 0; ; offset += 1000) {
    ...
    if (error)
      throw new Error(`Recommendation corpus is unavailable: ${error.message}`);
...
function normalizeProblem(
  ...
  const difficulty =
    typeof row.difficulty === "number" && [1, 2, 3].includes(row.difficulty)
      ? (row.difficulty as 1 | 2 | 3)
      : null;
  return {
    ...
    primaryTopics: Array.isArray(row.primary_topics)
      ? row.primary_topics.map(String).slice(0, 4)
      : [],
...
async function handle(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        available: false,
        results: [],
        error: "Log in to get personalized recommendations.",
      },
      { status: 401 },
    );
  ...
  try {
    const includeEmbeddings =
      input.mode === "continue" || input.mode === "harder";
    const [databaseProblems, progressResponse, nearDuplicates] =
      await Promise.all([
        readProblems(supabase, includeEmbeddings),
        supabase
          .from("problem_progress")
          .select("problem_id, bookmarked, completed, completed_at, updated_at")
          .eq("user_id", userData.user.id),
        readNearDuplicates(),
      ]);
    if (progressResponse.error)
      throw new Error(
        `Your progress is unavailable: ${progressResponse.error.message}`,
      );
    const progress = (
      (progressResponse.data ?? []) as Array<{
        problem_id: string;
        bookmarked: boolean;
        completed: boolean;
    ...
    ).map((row): RecommendationProgress => ({
      problemId: String(row.problem_id),
    ...
    const problems = databaseProblems.map((row) =>
      normalizeProblem(row, includeEmbeddings),
    );
    const results = buildRecommendations({
      ...
      limit: input.limit,
      nearDuplicates,
      excludeProblemIds: input.excludeProblemIds,
      seed: `${userData.user.id}:${new Date().toISOString().slice(0, 10)}`,
    });
