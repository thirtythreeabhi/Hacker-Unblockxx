...
function progressIsRelevant(message: string) {
  return /\b(progress|completed|complete|bookmarked|bookmark|weak|revisit|attempted)\b/i.test(
    message,
  );
}
...
async function progressFor(
  ...
  if (!ids.length)
    return new Map<string, { completed: boolean; bookmarked: boolean }>();
...
export async function POST(request: Request) {
  ...
  const history =
    typeof body.history === "string"
      ? body.history.trim().slice(0, 12000)
      : undefined;
  if (!mode || !message || message.length > 4000)
    return errorResponse(
      "mode and a message up to 4000 characters are required.",
      400,
    );

  const contestId = numericId(body.contestId);
  const contentId = numericId(body.contentId);
  if (mode === "current" && (!contestId || !contentId))
    return errorResponse(
      "Current-problem mode requires numeric contestId and contentId.",
      400,
    );

  const currentProblemId = numericId(body.currentProblemId);
  const focus: CorpusFocus =
    body.focus === "easier" || body.focus === "harder" ? body.focus : "related";
  ...
  try {
    ...
    if (authError || !authData.user)
      return errorResponse("Log in to use Ask HackerBlocks.", 401);

    if (mode === "current") {
      const upstream = await fetchQuestion(
        contentId as string,
        contestId as string,
      );
      if (!upstream.ok)
        return errorResponse(
          `Question could not be fetched (HTTP ${upstream.status}).`,
      ...
      const progress =
        progressIsRelevant(message) && question.problemId
    ...
    if (currentProblemId) {
      const { data: current, error: currentError } = await supabase
        .from("problem_search")
        .select("embedding, difficulty")
        .eq("problem_id", currentProblemId)
        .maybeSingle();
      if (currentError)
        return errorResponse(
          "Corpus search is not available until the search migration is applied.",
        );
      const literal = vectorLiteral(current?.embedding);
      if (!literal)
        return NextResponse.json({
          answer:
            "This problem does not have a usable corpus embedding yet, so I cannot search for grounded matches.",
          references: [],
          mode,
          retrievalCount: 0,
        });
    ...
    const filterDifficulty =
      focus === "easier" && currentDifficulty !== null && currentDifficulty > 1
        ? currentDifficulty - 1
        : focus === "harder" &&
    ...
    if (matchError)
      return errorResponse("Corpus search is temporarily unavailable.");

    const matchRows = ((matches as MatchRow[] | null) ?? []).filter(
      (row) => String(row.problem_id) !== currentProblemId,
    );
    const ids = matchRows.map((row) => String(row.problem_id));
    if (!ids.length)
      return NextResponse.json({
        answer: "No grounded corpus matches were found for that request.",
        references: [],
        mode,
        retrievalCount: 0,
      });
    ...
    const { data: details, error: detailsError } = await supabase
      ...
      .in("problem_id", ids);
    if (detailsError)
      return errorResponse("Corpus references are temporarily unavailable.");
    const detailsById = new Map(
      (details ?? []).map((row) => [String(row.problem_id), row]),
    );
    const progress = progressIsRelevant(message)
      ? await progressFor(supabase, authData.user.id, ids)
      : new Map<string, { completed: boolean; bookmarked: boolean }>();
      ...
      .map((reference) => {
        ...
        const description =
          typeof detail?.description === "string"
            ? detail.description.replace(/\s+/g, " ").trim().slice(0, 500)
            : "";
        return [
          ...
          reference.completed || reference.bookmarked
            ? `userStatus=${reference.completed ? "completed" : "bookmarked"}`
            : null,
        ]
          .filter(Boolean)
          .join(" | ");
    ...
    const messageText =
      error instanceof Error
        ? error.message
        : "Ask HackerBlocks is temporarily unavailable.";
    if (/configured|Gemini|embedding/i.test(messageText))
      return errorResponse(
        "Ask HackerBlocks is not configured or its AI provider is unavailable.",
      );
