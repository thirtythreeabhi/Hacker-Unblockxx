...
export async function embedQuery(query: string) {
  ...
  const cached = embeddingCache.get(
    `${embeddingModel}:${embeddingDimensions}:${normalized}`,
  );
  ...
  if (!keys.length)
    throw new Error("Semantic search is not configured on the server.");

  let lastError: Error | null = null;
  const maxRetries = Math.max(
    2,
    Number.parseInt(process.env.GEMINI_EMBEDDING_MAX_RETRIES ?? "3", 10) || 3,
  );
  ...
  for (let attempt = 0; attempt < maxRetries; attempt += 1) {
    ...
    try {
      const response = await fetch(
        `${API_ROOT}/${encodeURIComponent(embeddingModel)}:embedContent?key=${encodeURIComponent(key)}`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        (error as Error & { status?: number; transient?: boolean }).status =
          response.status;
        (error as Error & { status?: number; transient?: boolean }).transient =
      ...
      if (values.length !== embeddingDimensions)
        throw new Error(
          `Embedding dimension mismatch: expected ${embeddingDimensions}, received ${values.length}.`,
        );
      embeddingCache.set(
        `${embeddingModel}:${embeddingDimensions}:${normalized}`,
        { values, expiresAt: Date.now() + CACHE_TTL_MS },
      );
      return values;
    } catch (error) {
      lastError =
      ...
      const retryable =
        ...
        /network|fetch/i.test(lastError.message);
      if ((!retryable && !credentialFailure) || attempt >= maxRetries - 1)
        break;
      if (credentialFailure) continue;
      await new Promise((resolve) =>
        setTimeout(resolve, Math.min(30000, 750 * 2 ** attempt)),
      );
    } finally {
